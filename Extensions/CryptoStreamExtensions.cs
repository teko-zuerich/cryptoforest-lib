...
internal static class CryptoStreamExtensions
{
    ...
    internal static async Task DecryptFileAsync(this CryptoStream cryptoStream, Stream writeStream, long fileLength, CancellationToken cancellationToken = default)
    {
    ...
    internal static async Task SkipFileAsnyc(this CryptoStream cryptoStream, long fileLength, CancellationToken cancellationToken = default)
    {
        var remainingFileLength = fileLength;
        var currentFileData = new byte[remainingFileLength < 10485760 ? remainingFileLength : 10485760];
        var currentlyRead = await cryptoStream.ReadAsync(currentFileData, cancellationToken);
